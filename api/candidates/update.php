...
try {
    ...
    $dupStmt = $pdo->prepare("SELECT id FROM candidates WHERE LOWER(firstname) = LOWER(?) AND LOWER(lastname) = LOWER(?) AND id != ?");
    $dupStmt->execute([$firstname, $lastname, $id]);
    if ($dupStmt->fetch()) {
        http_response_code(400);
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        logCandidateActivity($_SESSION['user_id'], 'update', 'Updated candidate: ' . $candidateName . ' (ID: ' . $id . ')');
    }
